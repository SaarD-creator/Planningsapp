...
def maak_planning(studenten_local):
    student_dict = {s["naam"]: s for s in studenten_local}

    # Pauzevlinders
    pauzevlinder_namen = [str(ws[f'BN{rij}'].value).strip() for rij in range(4,11) if ws[f'BN{rij}'].value]
    ...
    # --- First pass: generate initial assignments per attractie using plan_attractie_pos ---
    for attractie in attracties_te_plannen:
        posities = [plan_attractie_pos(
        ...
        if aantallen.get(attractie,1) >= 2:
            posities.append(plan_attractie_pos(
                attractie, studenten_local, student_bezet,
                gebruik_per_attractie_student[attractie], open_uren, dagplanning
            ))
        dagplanning[attractie] = posities

    # --- Enforce first-spot priority globally ---
    for uur in open_uren:
        uur_bezet = set()
        for attr, posities in dagplanning.items():
            for pos in posities:
                if pos.get(uur,"") not in ["","NIEMAND"]:
                    uur_bezet.add(pos[uur])

        # Fill first positions
        for attr, posities in dagplanning.items():
            pos = posities[0]
            if pos.get(uur,"") in ["","NIEMAND"]:
                kandidaten = [
                    ...
                    and s["naam"] not in uur_bezet
                    and gebruik_per_attractie_student[attr][s["naam"]] < 6
                ]
                if kandidaten:
                    ...
                    uur_bezet.add(gekozen["naam"])

        # Fill second positions if students remain
        for attr, posities in dagplanning.items():
            if len(posities) < 2:
                continue
            pos = posities[1]
    ...
    for uur in open_uren:
        ...
        candidates = [
            s["naam"] for s in studenten_local
            if uur in s["uren_beschikbaar"] and s["naam"] not in uur_bezet and not s.get("is_pauzevlinder")
        ]
        tried_guard = 0
        while candidates and lege_posities:
            tried_guard += 1
            if tried_guard > 2000: break
            extra_student = candidates.pop(0)
            geplaatst = False
            for attr,pos in list(lege_posities):
                if extra_student in pos.values(): continue
                if attr in student_dict[extra_student]["attracties"]:
                    pos[uur] = extra_student
                    student_bezet[extra_student].append(uur)
                    gebruik_per_attractie_student[attr][extra_student] += 1
                    uur_bezet.add(extra_student)
                    lege_posities.remove((attr,pos))
                    geplaatst = True
                    break
            if not geplaatst:
                candidates.append(extra_student)
                break

    # --- Build extra_per_uur in same format as original script ---
    extra_per_uur_out = defaultdict(list)
    for uur in open_uren:
        for s in studenten_local:
            if uur in s["uren_beschikbaar"] and s["naam"] not in student_bezet or s.get("is_pauzevlinder"):
                continue
            assigned_somewhere = any(pos.get(uur,"") == s["naam"] for posities in dagplanning.values() for pos in posities)
            if not assigned_somewhere:
                extra_per_uur_out[uur].append(s["naam"])

    return dagplanning, extra_per_uur_out, selected

# -----------------------------
# Herhaal tot volledige planning (sneller)
# -----------------------------
max_attempts = 150
for attempt in range(max_attempts):
    # Shallow copy each student dict (faster than deepcopy)
    studenten_copy = [s.copy() for s in studenten]
    dagplanning, extra_per_uur, selected = maak_planning(studenten_copy)
    
    # Check if all spots filled or no extras left
    volledig = True
    ...
    if volledig:
        # Update studenten with the successfully planned copy
