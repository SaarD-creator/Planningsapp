...
def flexible_block_partition(L):
    """
    Verdeel een shift van lengte L in flexibele blokken.
    Prioriteit: 3 > 2 > 4 > 1 uur
    """
    blocks = []
    remaining = L
    while remaining > 0:
...
for s in studenten_sorted:
    ...
    for run in runs:
        ...
        if L == 0:
            continue
        block_sizes = flexible_block_partition(L)
        start_idx = 0
