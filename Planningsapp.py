...
# -----------------------------
## Fase 2 & 3: alle studenten (geen pauzevlinders) via registry-gestuurde plaats_student
studenten_te_plannen = [s for s in min_werkers if s["naam"] not in [pv["naam"] for pv in selected]]
niet_geplaatst = []
for s in studenten_te_plannen:
    if not plaats_student(s, harde_mode=True):
        niet_geplaatst.append(s)
